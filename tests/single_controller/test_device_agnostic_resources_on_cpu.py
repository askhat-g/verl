...
or by assuming NVML is present. They run without a Ray cluster and without an
accelerator: the Ray state APIs and the active platform are stubbed out.
"""
...
def test_set_numa_affinity_skips_without_cuda(monkeypatch):
    """NUMA pinning goes through NVML, so non-CUDA accelerators must return early."""
    monkeypatch.setattr(distributed_utils, "is_cuda_available", False)

    def _fail(*args, **kwargs):
        raise AssertionError("libnuma/NVML must not be touched on a non-CUDA platform")
...
def test_set_numa_affinity_attempts_setup_on_cuda(monkeypatch):
    """On CUDA the NVML path is still entered (and failures stay non-fatal)."""
    monkeypatch.setattr(distributed_utils, "is_cuda_available", True)
    calls = []
